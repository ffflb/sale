...
<script>
    ...
    function getData() {
        let jsonData;
        jsonData = ${requestScope.orderAndSorts}
    ...
    layui.use('table', function(){
        ...
        table.on('tool(ID-table-demo-data)', function(obj) {
            ...
            if (obj.event === 'del') { //删除
                ...
                layer.confirm('真的删除吗', function (index) {
                    ...
                    $.ajax({
                        type:"POST",
                        url:"${pageContext.request.contextPath}/order/delOrderStatus",
                        data:{id},
                        async:false,
                        success:function (data){
                            layer.msg('删除成功', {icon: 1});
                            LoadIndex("${pageContext.request.contextPath}/order/toOrderTable")
                        },
                        error: function() {
        ...
        $('#delBatchOrder').click(function(){
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/order/delBatchOrderStatus?'+ids,
                type: "get",
                data: {idArray},
                success: function () {
                    LoadIndex("${pageContext.request.contextPath}/order/toOrderTable")
    ...
    layui.use(['form'], function () {
        ...
        form.on('submit(demo1)', function (data) {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/order/toOrderTable',
                type: "post",
                data: {orderName},
                success: function () {
                    LoadIndex("${pageContext.request.contextPath}/order/toOrderTable?orderName="+orderName)
                    layer.msg('搜索成功');
                },
                error: function () {
                    layer.msg('搜索失败');
